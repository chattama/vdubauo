...
void CExceptionHardware::se_translator_function(unsigned int u, EXCEPTION_POINTERS* pException)
{
    ...
    case EXCEPTION_ACCESS_VIOLATION: 
        {
            ...
            if (this->pExceptionInformation->ExceptionRecord->NumberParameters>=2)
            {
                if (this->pExceptionInformation->ExceptionRecord->ExceptionInformation[0]==0)
                    _stprintf(psz,_T("ACCESS_VIOLATION reading location %p"),this->pExceptionInformation->ExceptionRecord->ExceptionInformation[1]);
                else
                    _stprintf(psz,_T("ACCESS_VIOLATION writing location %p"),this->pExceptionInformation->ExceptionRecord->ExceptionInformation[1]);
            }
