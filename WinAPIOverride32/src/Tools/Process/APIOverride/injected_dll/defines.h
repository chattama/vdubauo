...
#define ESP_SECURITY_SIZE 100 // this number is the number of args that can be unfortunately forget in config file
...
#define UNHOOK_MAX_WAIT_TIME 5000 // max time (in ms) for executing hook, used as max wait time for unhooking
...
#define ASM_CALL_INSRUCTION_SIZE 5 // quite wrong with CALL dword ptr FF 15 --> size = 6
#define HOOK_NUMBER_OF_EBP_PUSH  2 // number of ebp content and return address we should not take into account (until we are inside APIHandler function)
