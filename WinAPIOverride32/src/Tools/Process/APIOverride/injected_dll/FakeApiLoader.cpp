...
BOOL LoadFakeApiDll(TCHAR* szFakeAPIDLLFullPath)
{
    ...
    for (cnt=0;cnt<3;cnt++)
    {
        ...
        if (ArrayFakingArray[cnt])
            LoadFakeArray(pFakingDllInfos,szFakeAPIDLLFullPath,ArrayFakingArray[cnt],iFakeAPIEncoding,FakingDllArray);
    }
...
//-----------------------------------------------------------------------------
#if 0
BOOL LoadFakeArray(FAKING_DLL_INFOS* pFakingDllInfos,TCHAR* szFakeAPIDLLFullPath,PVOID FakeArray,int iFakeAPIEncoding,tagFakingDllArray FakingDllArray)
{
    int                      Index;
    BOOL                     bNoMoreItems;
    ...
    while(!bNoMoreItems)
    {
        ...
        if (IsCOMHookDefinition(FakeApiInfos.pszModuleName))
        {
            ...
            if (pComManager)
            {
                ...
                // if a faking hook was already installed for this COM definition
                if (pAlreadyHookedFakingDllInfos)
                {
                    ...
                    // report the error
                    _sntprintf(pszMsg,3*MAX_PATH,_T("A Fake API was already defined for\r\nmodule %s\r\nand API name %s"),FakeApiInfos.pszModuleName,FakeApiInfos.pszAPIName);

                    // get file name
                    TCHAR pszFileName[MAX_PATH];
                    if (GetFakeApiDllName(pAlreadyHookedFakingDllInfos,pszFileName))
                    {
                        _tcscat(pszMsg,_T("\r\nby "));
                        _tcscat(pszMsg,pszFileName);
                    }
                    _tcscat(pszMsg,_T("\r\nFaking dll can't be changed for COM hooking,\r\nso you must unload other FAKING dll before loading this new one"));
                    DynamicMessageBoxInDefaultStation(NULL,pszMsg,_T("Error"),MB_ICONERROR|MB_YESNO|MB_TOPMOST);
                }
            }
        }
        else // not COM definition
        {
            // do the job for specified struct
            LoadFakeAPIDefinition(pFakingDllInfos,&FakeApiInfos,FakingDllArray);
        }
                           ...
                           tagFakingDllArray FakingDllArray)
{
    ...
    if (IsBadCodePtr((FARPROC)pbAPI))
    {
        ...
        _sntprintf(pszMsg,
                   3*MAX_PATH,
                   _T("Bad code pointer 0x%p for function %s in dll\r\n%s\r\nFunction not hooked"),
                   pbAPI,
    ...
    if (!bAlreadyHooked)
    {
        ...
        pAPIInfo->FirstBytesCanExecuteAnywhereSize=pFakeApiInfos->FirstBytesCanExecuteAnywhereSize;
    ...
    if (bAlreadyHooked)
    {
        ...
        else
        {
            ...
            // ask for update fake api
            _sntprintf(pszMsg,3*MAX_PATH,_T("A Fake API was already defined for\r\nmodule %s\r\nand API name %s"),pFakeApiInfos->pszModuleName,pFakeApiInfos->pszAPIName);

            // get file name
            TCHAR pszFileName[MAX_PATH];
            if (GetFakeApiDllName(pAPIInfo->pFakeDllInfos,pszFileName))
            {
                _tcscat(pszMsg,_T("\r\nby "));
                _tcscat(pszMsg,pszFileName);
            }
...
begin:
    for (pItemAPIInfo = pLinkListAPIInfos->Head;pItemAPIInfo ;pItemAPIInfo = pNextItemAPIInfo)
    {
        ...
        if (bUnhookIfPossible)
        {
            ...
            // assume pNextItemAPIInfo as not been destroyed during lock release
            if (!IsBadReadPtr(pNextItemAPIInfo,sizeof(CLinkListItem)))
                goto begin;
        }
    }
    pLinkListAPIInfos->Unlock();
