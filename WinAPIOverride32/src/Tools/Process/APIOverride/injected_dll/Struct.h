...
typedef struct tagAPI_INFO
{
    ...
    BYTE OpcodesExecutedAtAnotherPlace[FIRST_OPCODES_MAX_SIZE];// opcode executed at another place if FirstBytesCanExecuteAnywhereSize!=0
    ...
    DWORD dwUseCount;           // number of time the function is being hooked
	...
	BOOL DontCheckModulesFilters;// use for EXE_INTERNAL@0x or DLL_INTERNAL@0x, because this can be callback of Enum functions
								 // like EnumWindows, and in this case caller address is in the user32.dll
								 // memory space, so if we don't have this flag, callback will 
								 // never appears until we stop using filters
